...
/**
 *   @file mozfile_module.c		JS File object, imported more-or-less as-is
 *					from Spidermonkey, Feb 2009 hg tip.
 *
 *   This is a terrible, terrible example of how to build a module. Also the
 *   code quality here is quite low. I fully intend to replace this with a
 ...
 *		internals.  If you hit a castrated method, don't worry, it will
...
#undef JS_PUBLIC_API
#define JS_PUBLIC_API(a) static a
...
#define JSFILE_CHECK_ONE_ARG(op)                                              \
    if (argc != 1) {                                                          \
        char astr[NUMBER_SIZE];                                               \
        sprintf(astr, "%d", argc);                                            \
        JS_ReportErrorNumber(cx, JSFile_GetErrorMessage, NULL,                \
                             JSFILEMSG_EXPECTS_ONE_ARG_ERROR, op, astr);      \
        goto out;                                                             \
    }
...
JS_PUBLIC_API(JSObject*) js_NewFileObject(JSContext *cx, const char *filename);
...
js_fileBaseName(JSContext *cx, const char *pathname)
{
    ...
    idx = strlen(pathname)-1;

    /* Chop off trailing separators. */
    while (idx > 0 && (pathname[idx]==FILESEPARATOR ||
                         pathname[idx]==FILESEPARATOR2)) {
        --idx;
    }
    ...
    /* Allocate and copy. */
    result = JS_malloc(cx, aux - idx + 1);
    if (!result)
        return NULL;
    strncpy(result, pathname + idx + 1, aux - idx);
    result[aux - idx] = '\0';
    return result;
}
...
js_absolutePath(JSContext *cx, const char * path)
{
    ...
    if (js_isAbsolute(path)) {
        ...
        obj = myModuleObject;
/*** xxx this is broken because we don't know constructor name anymore. */
	return JS_strdup(cx, __FILE__ " js_absolutePath not fully implemented");
...
static JSBool
js_FileOpen(JSContext *cx, JSObject *obj, JSFile *file, const char *mode){
    JSString *type, *mask;
    jsval v[2];
    jsval rval;
...
file_open(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    if (file->path[0]==PIPE_SYMBOL || file->path[len-1]==PIPE_SYMBOL) {
        ...
        /* xxx wes: set them to 0666 and let umask sort it out */
        file->handle = PR_Open(file->path, mask, 0666);
    }
...
file_copyTo(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    /* xxx wes: should probably use src perms for dest */
    handle = PR_Open(dest, PR_WRONLY|PR_CREATE_FILE|PR_TRUNCATE, 0666);
...
file_read(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    if (count>0) {
        JSString *str = JS_NewUCStringCopyN(cx, buf, count);
...
file_readln(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    for (;;) {
        bytesRead = js_FileRead(cx, file, &data, 1, file->type);
        if (bytesRead < 0)
            goto out;
        if (bytesRead == 0)
            goto eof;
        ...
        switch (data) {
          case '\r':
            bytesRead = js_FileRead(cx, file, &data2, 1, file->type);
            if (bytesRead < 0)
                goto out;

            if (bytesRead == 1 && data2 != '\n') {
...
file_list(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    if (argc==1) {
      return gpsee_throw(cx, MODULE_ID ".list.notImplemented: list method with argc==1 unavailable");
#if 0 /* xxx */
    ...
    while ((entry = PR_ReadDir(dir, PR_SKIP_BOTH))!=NULL) {
        ...
        if (re!=NULL) {
	  return gpsee_throw(cx, MODULE_ID ".list.regexp.notImplemented: methods with regexp unavailable");
#if 0 /* xxx */
            size_t index = 0;
...
file_toURL(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    *rval = STRING_TO_JSVAL(JS_NewStringCopyZ(cx, url));

    /* TODO: js_escape in jsstr.h may go away at some point */
/* xxx   return js_str_escape(cx, obj, 0, rval, rval); */
  return gpsee_throw(cx, MODULE_ID, ".toURL.notImplemented");
  out:
  return JS_FALSE;	/* out needed for JSFILE_CHECK_NATIVE */
}
...
*/
static JSFile*
file_init(JSContext *cx, JSObject *obj, const char *bytes)
{
    JSFile *file;
...
js_NewFileObject(JSContext *cx, const char *filename)
{
    JSObject    *obj;
    JSFile      *file;
...
js_NewFileObjectFromFILE(JSContext *cx, FILE *nativehandle, char *filename,
    int32 mode, JSBool isOpen, JSBool randomAccess)
{
    JSObject *obj;
    JSFile   *file;
...
/* xxx	PRFileInfo  info; */
    JSBool      flag;
/* xxx	PRExplodedTime expandedTime; */
    ...
    case FILE_MODE:
        ...
        *vp = STRING_TO_JSVAL(JS_NewStringCopyZ(cx, bytes));
        ...
        break;
    case FILE_CREATED:
      return gpsee_throw(cx, MODULE_ID ".property.creationTime.notImplemented: date-returning properties not implemented");
#if 0 /* xxx */
        ...
        *vp = OBJECT_TO_JSVAL(js_NewDateObject(cx,  expandedTime.tm_year,
        ...
        break;
    case FILE_MODIFIED:
      return gpsee_throw(cx, MODULE_ID ".property.lastModified.notImplemented: date-returning properties not implemented");
#if 0 /* xxx */
...
/* Declare class */
JSClass js_FileClass = {
    MODULE_ID ".File", JSCLASS_HAS_PRIVATE/* | JSCLASS_HAS_CACHED_PROTO(JSProto_File)*/,
...
js_InitFileClass(JSContext *cx, JSObject* obj)
{
    ...
    js_FileClass.name = MODULE_ID ".File";
...
#else
    {
      const char patch[] = 
"(function(ctor) {\n"
"  ctor.output = new ctor(\"/dev/stdout\");\n"
"  ctor.output.open(\"write\");\n"
"  ctor.input = new ctor(\"/dev/stdin\");\n"
"  ctor.input.open(\"read\");\n"
"  ctor.error = new ctor(\"/dev/stderr\");\n"
"  ctor.error.open(\"write\");\n"
"}(this));\n";

      JSScript *script = JS_CompileScript(cx, ctor, patch, sizeof(patch) - 1, MODULE_ID "_patch", 1);
      JSObject *scrobj;
      jsval	rval;
	...
	return NULL;
      ...
      JS_AddNamedRoot(cx, &scrobj, MODULE_ID "_patch_scrobj");
      JS_ExecuteScript(cx, ctor, script, &rval);
...
const char *mozfile_InitModule(JSContext *cx, JSObject *moduleObject)
{
  JSObject *proto;

  myModuleObject = moduleObject;

  proto = js_InitFileClass(cx, moduleObject);
  if (!proto)
    return NULL;
...
JSBool mozfile_FiniModule(JSContext *cx, JSObject *moduleObject)
{
  myModuleObject = NULL;
  return JS_TRUE;
}
