...
function errorReporterHook(msg, filename, lineno, lineBuf, tokenOffset)
{
    ...
    if (filename == "jsdb_eval")
    {
      print("Error: " + msg);
      return jsd.JSD_ERROR_REPORTER_CLEAR_RETURN;
    }
...
{
    if(!handle) {
        throw new Error("JSDScript: null handle?");
    }
    this.handle = handle;
    this.url = url;
...
{
    if(!script)
        throw new Error("Script undefined/falsey");
...
{
    if(!handle) {
        throw new Error("JSDFrame: null handle?");
    }
    this.handle = handle;
...
function list()
{
    ...
    if (require("fs-base").exists(filename))
      filename = require("fs-base").canonical(filename);
...
var print = print ? print : function print()
{
    var i;
    var str = "";
...
var evalLoopPrompt = "gsrdb-self"+(jsd.DebuggerDepth - 1)+"> ";
var evalLoopFilename = "gsrdb-self"+(jsd.DebuggerDepth - 1);
...
if (jsd.DebuggerDepth === 1)
  doEvalLoop = gsrdb_evalLoop;
else
  doEvalLoop = jsdb_evalLoop;
